...
export const verifyEmail: RequestHandler = async (req, res) => {
  ...
  if (!isMatched)
    return sendErrRes(res, "unauthorized request, invalid token!", 403);
...
export const grantAccessToken: RequestHandler = async (req, res) => {
  ...
  if (!payload.id) return sendErrRes(res, "Unauthorized request!", 401);
...
export const signOut: RequestHandler = async (req, res) => {
  ...
  if (!user)
    return sendErrRes(res, "Unauthorized request, user not found!", 403);
...
export const updatePassword: RequestHandler = async (req, res) => {
  ...
  if (!matched)
    return sendErrRes(res, "The new password must be different!", 422);
...
export const updateProfile: RequestHandler = async (req, res) => {
  ...
  if (typeof name !== "string" || name.trim().length < 3) {
    return sendErrRes(res, "Invalid name!", 422);
  }
